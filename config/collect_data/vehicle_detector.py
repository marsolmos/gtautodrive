...
DATA_DIR = os.path.join(os.getcwd(), 'vehicle_detector_data')
...
while True:
    ...
    time1 = time.time()
    print('Object detection - Inference time (s): {}'.format(time1-time0))
    ...
    for i,b in enumerate(detections['detection_boxes'][0]):
        ...
        if detections['detection_classes'][0][i] == 2 or detections['detection_classes'][0][i] == 5 or detections['detection_classes'][0][i] == 7:
            if detections['detection_scores'][0][i] >= 0.5:
                ...
                if apx_distance <= 0.5:
                    if mid_x > 0.3 and mid_x < 0.7 and aspect_ratio < 2:
                        print(aspect_ratio)
